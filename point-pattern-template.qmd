...
```{r}
voles_density <- density(voles_full, sigma = 8000) ### try different sigmas
...
Changed the resolution of the lo
...
```{r}
class(gfunction_out) #checking class
head(gfunction_out) #column names 
...
```{r}
r_vec <- seq(0, 800000, 5000)

lfunction_out <- envelope(voles_full, fun = Lest, r = r_vec, nsim=20)
